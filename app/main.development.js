...
import { createLogger, lastLogLine, GetHxdLogs, GetDcrwalletLogs } from "./main_dev/logging";
...
import { cleanShutdown, GetHxdPID, GetDcrwPID } from "./main_dev/launch";
...
ipcMain.on("clean-shutdown", async function(event){
  const stopped = await cleanShutdown(mainWindow, app, GetHxdPID(), GetDcrwPID());
...
ipcMain.on("get-hxd-logs", (event) => {
  event.returnValue = GetHxdLogs();
});
...
ipcMain.on("get-last-log-line-hxd", event => {
  event.returnValue = lastLogLine(GetHxdLogs());
});
...
app.on("before-quit", (event) => {
  ...
  cleanShutdown(mainWindow, app, GetHxdPID(), GetDcrwPID());
