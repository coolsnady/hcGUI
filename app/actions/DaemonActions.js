...
export const DELETE_HXD_ATTEMPT = "DELETE_HXD_ATTEMPT";
export const DELETE_HXD_FAILED = "DELETE_HXD_FAILED";
export const DELETE_HXD_SUCCESS = "DELETE_HXD_SUCCESS";
...
export const deleteDaemonData = () => (dispatch, getState) => {
  ...
  dispatch({ type: DELETE_HXD_ATTEMPT });
  wallet.deleteDaemonData(appData, isTestNet(getState()))
    .then(() => {
      dispatch({ type: DELETE_HXD_SUCCESS });
      dispatch(shutdownApp());
    })
    .catch((err) => dispatch({ err, type: DELETE_HXD_FAILED }));
};
