...
import { getWalletPath, getWalletDBPathFromWallets, getHxdPath, hxdCfg, dcrctlCfg, appDataDirectory, getExecutablePath } from "./paths";
import { createTempHxdConf, initWalletCfg, newWalletConfigCreation, getWalletCfg } from "../config";
import { launchHXD, launchDCRWallet, GetHxdPID, GetDcrwPID, closeDCRW, GetDcrwPort } from "./launch";
...
export const deleteDaemon = (appData, testnet) => {
  let removeDaemonDirectory = getHxdPath();
...
export const startDaemon = (mainWindow, daemonIsAdvanced, primaryInstance, appData, testnet, reactIPC) => {
  if (GetHxdPID() && GetHxdPID() !== -1) {
    logger.log("info", "Skipping restart of daemon as it is already running " + GetHxdPID());
    return GetHxdPID();
  }
  if(appData){
  ...
  if (!daemonIsAdvanced && !primaryInstance) {
    logger.log("info", "Running on secondary instance. Assuming hxd is already running.");
    let hxdConfPath = getHxdPath();
    if (!fs.existsSync(hxdCfg(hxdConfPath))) {
      hxdConfPath = createTempHxdConf();
    }
    return -1;
  }
  try {
    ...
    return launchHXD(mainWindow, daemonIsAdvanced, hxdConfPath, appData, testnet, reactIPC);
  } catch (e) {
